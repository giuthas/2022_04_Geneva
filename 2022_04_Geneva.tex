...
\frame{\frametitle{Foreshadowing} 
  ...
  \item Methods: Algorithms for processing ultrasound
...
\frame{\frametitle{Introduction I} 
  ...
  \begin{itemize}
  \item Speech RTs are usually measured based on acoustics even though this is
    known to be problematic \citep{Rastle:CMN:2002,Rastle:CME:2005}.
...
\frame{\frametitle{Algorithms 1: Pixel Difference (PD)} 
...
\frame{\frametitle{Algorithms 2: Automated onset detection} 
  \begin{itemize}
  \item Looking at clear examples like the previous one, it would seem
  reasonable to automate the onset detection task.
  \item Reality -- as usual -- is a bit more complex than that and simple
  automation tends to produce unreliable results even for fairly clear cases.
  \item The solution is broadly as follows:
    \begin{itemize}
      \item Calculate PD for each scanline -- 63 in this data. (This is called
      Scanline Based Pixel Difference or SBPD).
      \item Use Dynamice Time Warping to find the movement onset for each
      scanline.
      \item Use the median of these as the onset for a given trial.
    \end{itemize}
  \item This approach has been validated against other methods of detecting
    movement onset in ultrasound data \citep{Palo:MPS:2019}.
  \end{itemize}
}
...
\frame{\frametitle{Scanline Based Pixel Difference (SBPD)}
